...
$_GET['the_stream']=preg_replace('/[^a-z0-9-_:\/.;]+/i', '', $_GET['the_stream']);
					...
					if (!$fp) {
						...
						if (stripos($info,'<title>Shoutcast') || stripos($info,'<title>Redirect</title>')) { //Is shoutcas ?
							...
							if (trim($info)) {
								...
								//$info="754,1,874,4000,744,128,Janet, Jackson - Dammn, Baby";
								//$info="754,1,874,4000,744,128,Smokey, Robinson - Dammn, Baby";
								$split = explode(',', $info);
								if (empty($split[6]) ) {
									$songtitle="The song title is not available"; // sever is online but no song title
								} else {
									$split = array_slice($split, 6);
									$title = implode(" ",$split);
									  ...
									  if (trim($songtitle)!='') {
										...
										if(!$fp) {
										  ...
										  $songtitle = "The song title is not available - Unable to connect to Icecast server.";
												...
												$search_start_point=stripos($stats_file_contents,'yp_currently_playing');
												if ($search_start_point>0) {
													$stats_file_contents=substr ( $stats_file_contents,$search_start_point );
													$search_end_point=stripos($stats_file_contents,'"},{"');
													$stats_file_contents=substr ( $stats_file_contents,23, $search_end_point-23);
